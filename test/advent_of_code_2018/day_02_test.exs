defmodule AdventOfCode.Day02Test do
  ...
  test "examples part 1" do
    r1 = [1, 0, 0, 0, 99] |> Memory.to_map() |> run(0) |> Memory.to_list()
    assert r1 == [2, 0, 0, 0, 99]

    r2 = [2, 3, 0, 3, 99] |> Memory.to_map() |> run(0) |> Memory.to_list()
    assert r2 == [2, 3, 0, 6, 99]

    r3 = [2, 4, 4, 5, 99, 0] |> Memory.to_map() |> run(0) |> Memory.to_list()
    assert r3 == [2, 4, 4, 5, 99, 9801]

    r4 = [1, 1, 1, 4, 99, 5, 6, 0, 99] |> Memory.to_map() |> run(0) |> Memory.to_list()
